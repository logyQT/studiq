...
_All 3 items implemented in a single pass._
...
_Phase 2.1 completed. Remaining items not yet started._
...
- No backend changes — the session API already returns `flashcard_topic_assignments` in the flashcard data
...
**Fix:** The `fetchDueCards` async function already receives `deckIds` as a parameter. Add `topicIds` from the session props and pass both to the API call.
...
- Display: horizontal bar chart or simple list with accuracy %, similar to the teacher `byTopic` view

---

## Phase 3 — Card Features & Portability

_Bigger features that change what a card can be._
...
### 3.4 APKG Import/Export ⏳ DEFERRED - not a critical feature
...
└── 2.6 Topic UX Improvements
    ├── 2.6.1 Topic badges in session (frontend only)
    ├── 2.6.2 Fix session refetch filters (frontend only)
    └── 2.6.3 Student topic progress (needs new route + service)
