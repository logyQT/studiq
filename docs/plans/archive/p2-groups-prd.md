...
1. **Teachers see only their own content** — a teacher sees content they created, even within groups they manage or co-teach. (Corrected 2026-09-20: the original wording below — "teachers see content from groups they belong to + own personal content" — was never implemented and is now confirmed as intentionally superseded. The `teacher` org role is seeded with `'own'` scope for `flashcard.read`/`question.read`/etc. in [20260707000006_handle_new_organization_trigger.sql](../../../supabase/migrations/20260707000006_handle_new_organization_trigger.sql) — its own inline comment already called this "Educator — CRUD over own content" at seed time. Rationale: two teachers can co-manage the same group while teaching different subjects, so surfacing every co-teacher's materials by default would be noise rather than useful sharing. A teacher who wants to share content with a specific colleague does so explicitly, not implicitly via shared group membership.)
...
> **Note (2026-09-15, resolved 2026-09-20):** This document predates several changes and no longer matches the implementation 1:1 — e.g. `groups`/`group_members` (not `study_groups`), teacher group creation and ownership (PR #64, #66), group-based issue reporting and member management (PR #68). The discrepancy flagged here between `permissions.ts`'s `'own'` scope for the `teacher` role and this doc's original `'group'`-scope principle #1 has been resolved: `'own'` is the confirmed, intentional behavior (see principle #1 above) — the doc was wrong, not the code.
