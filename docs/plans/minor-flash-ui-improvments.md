...
- [x] Remove inline edit/delete buttons from cards → context menu on both platforms
- [x] Remove mobile "Manage" dropdown → context menu + FAB replaces all its actions
- [x] Add floating "+" FAB for new deck on both platforms (bottom-right fixed position)
- [x] Add context menu with: Select, Edit, Delete, Import CSV, Export CSV
- [x] Improve empty state — currently bare text, needs icon + "Create your first deck" CTA button
- [x] Extract `GRADIENTS` / `getGradient` to shared utility (duplicated in deck-detail-screen)

## 3. Deck Detail (`deck-detail-screen.tsx`)

- [x] Move header action buttons (edit, import, export, delete) to context menu
- [x] Fix flashcard count — currently shows `flashcards.length` (loaded slice), should use `currentDeck.flashcard_count` or API total
- [ ] Add back-to-top button after scrolling through many cards via infinite scroll
- [x] Improve empty state with descriptive subtitle ("This deck is empty. Add your first flashcard to start studying.")
- [x] Extract `GRADIENTS` / `getGradient` to shared utility
...
- [x] Extract duplicated utilities — `GRADIENTS`/`getGradient` (2 copies), `TOPIC_COLORS`/`getTopicColor` (2 copies)
