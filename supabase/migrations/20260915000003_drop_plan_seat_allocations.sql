-- ==========================================================
-- Migration: Drop default seat-pool seeding (Phase B.5 — S1)
--
-- Removes the `plan_seat_allocations` table and the pool-
-- creation section from `handle_new_organization`.  After
-- this migration, orgs get zero seat pools at creation time;
-- pools are created on-demand when seats are purchased.
-- ==========================================================

-- 1. Drop the allocations table (no FKs depend on it)
DROP TABLE IF EXISTS public.plan_seat_allocations;

-- 2. Rebuild handle_new_organization WITHOUT the seat-pool seeding
