-- ==========================================================
-- Migration: 20260915000008
-- Description: fix group-visibility scoping in the spaced-repetition
-- RPCs. get_due_flashcards, get_due_breakdown,
-- get_practice_state_breakdown and count_new_cards all treated
-- flashcards.visibility = 'group' as "visible to anyone in the
-- organization" — they never checked whether the requesting user is
-- actually a member of the specific group the card's deck was shared
-- with (via deck_groups). That's overly permissive: a student in ANY
-- group of the org would see spaced-repetition cards from decks
-- shared to groups they aren't in.
--
-- get_accessible_flashcards (61_content_groups.sql) already does this
-- correctly — join deck_groups -> group_members filtered by
-- p_user_id — this migration applies the same pattern to the practice
-- RPCs. No signature changes: p_user_id is already a parameter on all
-- four, so no TS caller changes are needed either.
-- ==========================================================
    ...
    WHERE
      CASE p_filter_type
        ...
        WHEN 'university' THEN f.organization_id = p_organization_id AND f.visibility = 'group'
          AND EXISTS (
            SELECT 1 FROM deck_groups dg
            JOIN group_members gm ON gm.group_id = dg.group_id
            WHERE dg.deck_id = f.deck_id AND gm.user_id = p_user_id
          )
        ELSE FALSE
      END
  ...
  WHERE rs.next_review_at <= NOW()
    ...
    AND (
      ...
      OR (p_created_by IS NULL AND p_organization_id IS NOT NULL AND f.organization_id = p_organization_id AND f.visibility = 'group'
          AND EXISTS (
            SELECT 1 FROM public.deck_groups dg
