-- Add p_new_only parameter to get_due_flashcards
-- When true, returns only new cards (no review state row) up to p_new_card_limit
-- Used by the "Learn New" tab in the unified study page

DROP FUNCTION IF EXISTS public.get_due_flashcards(
  p_user_id UUID, p_filter_type TEXT, p_university_id UUID,
  p_limit INTEGER, p_deck_ids UUID[], p_topic_ids UUID[], p_new_card_limit INTEGER
);
