...
export default function ClientSessionTest() {
  ...
  if (loading)
    return <div className="p-8 animate-pulse text-muted-foreground">Sprawdzanie sesji...</div>;
    ...
    <div className="p-8 bg-background text-foreground min-h-screen">
      <Card className="max-w-2xl mx-auto border-border bg-card">
        ...
        <CardContent className="space-y-6">
          <div
            className={`p-4 rounded-md border ${session ? 'border-primary/50 bg-primary/5' : 'border-destructive/50 bg-destructive/5'}`}
          >
          ...
          {session && (
            <div className="space-y-4">
              <div className="text-sm text-muted-foreground">
                <p>
                  ID Użytkownika:{' '}
                  <span className="text-foreground font-mono">{session.user.id}</span>
                </p>
                <p>
                  Wygasa o:{' '}
