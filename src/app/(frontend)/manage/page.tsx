...
export default function ManageOverviewPage() {
  ...
  const recentMembers = [...members].sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()).slice(0, 5);

  return (
    <div className="space-y-8">
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <StatCard title={t('total_members')} value={loading ? '...' : members.length} icon={Users} />
        <StatCard title={t('teachers')} value={loading ? '...' : teachers.length} icon={GraduationCap} />
        <StatCard title={t('students')} value={loading ? '...' : students.length} icon={UserCheck} />
          ...
          <CardContent className="space-y-3">
            <Link href="/manage/invitations">
              <Button variant="outline" className="w-full justify-between">
                <span className="flex items-center gap-2"><Plus className="h-4 w-4" /> {t('invite_members')}</span>
                <ArrowRight className="h-4 w-4" />
              </Button>
            </Link>
            <Link href="/manage/members">
              <Button variant="outline" className="w-full justify-between">
                <span className="flex items-center gap-2"><Users className="h-4 w-4" /> {t('manage_members')}</span>
                <ArrowRight className="h-4 w-4" />
              </Button>
            </Link>
          </CardContent>
